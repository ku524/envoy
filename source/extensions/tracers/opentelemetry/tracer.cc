...
std::string Tracer::generateTraceIdV4() {
  // Generate random 128-bit trace ID (UUIDv4-style).
  return absl::StrCat(Hex::uint64ToHex(random_.random()), Hex::uint64ToHex(random_.random()));
}
...
std::string Tracer::generateTraceId() {
  ...
  case TraceIdPattern::UUID_V7:
    return generateTraceIdV7();
  case TraceIdPattern::UUID_V4:
  default:
